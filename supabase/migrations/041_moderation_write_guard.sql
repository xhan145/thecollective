-- 041_moderation_write_guard.sql — close the holes the adversarial review of
-- 039/040 confirmed. Additive. After 040.
--
-- CRITICAL (findings 1/3/4 + 2): moderation_status/moderation_reason/held/
--   removed were client-writable — proofs/feedback/practice_tips kept the
--   Supabase default whole-table UPDATE grant, and the UPDATE RLS policies
--   check ownership only (not columns). An author could PATCH their own row
--   back to 'clear' and un-hide reported/admin-removed content. Fix mirrors
--   036: revoke whole-table UPDATE, re-grant only the columns clients write.
-- IMPORTANT (5): admin dismiss wasn't durable — re-reporting re-hid content.
-- IMPORTANT (6): the spam_signal reporters term had no credibility gate, so 5
--   frivolous/spam-account reports could brigade a user's future content.
-- MINOR (7): submit_report had no is_demo guard.

-- 1) Column-lock UPDATE (rows are still gated by the existing RLS policies;
--    this gates WHICH COLUMNS an authenticated client may write).
revoke update on public.proofs        from authenticated, anon;
revoke update on public.feedback      from authenticated, anon;
revoke update on public.practice_tips from authenticated, anon;

-- proofs: the only legitimate client updates (persistFeedback status flip,
-- setProofOpen). moderation_status/moderation_reason/held/removed excluded.
grant update (status, open_for_contributions, contribution_focus)
  on public.proofs to authenticated;

-- feedback + practice_tips: no direct client UPDATE flow exists (helpful is
-- credited by the mark_feedback_helpful definer RPC), so grant nothing —
-- definer RPCs run as owner and are unaffected.

-- 2) Recompute (re-declared from 040; reporters term now counts only CREDIBLE
--    distinct reporters (own spam_signal < 40) so a ring of flagged accounts
--    cannot brigade the auto-hold). Self-heal unchanged (reason='spam' only).
...
begin
  ...
  select count(distinct r.reporter_id) into v_reporters
    ...
    where r.status = 'open' and r.created_at >= now() - interval '30 days'
      and coalesce(rp.spam_signal, 0) < 40   -- credible reporters only
      and (
...
-- 3) submit_report (re-declared from 040): reject demo targets; severe hide
--    only fires while THIS reporter's report is 'open' (so an admin dismiss is
--    durable — re-reporting the same content no longer re-hides it). Mild path
--    already counts open reports only.
...
begin
  ...
  if p_target_type = 'proof' then
    select user_id, is_demo into v_author, v_is_demo from public.proofs where id = p_target_id;
  else
    select author_id, is_demo into v_author, v_is_demo from public.feedback where id = p_target_id;
  end if;
  if v_author is null then raise exception 'TARGET_NOT_FOUND'; end if;
  if coalesce(v_is_demo, false) then raise exception 'CANNOT_REPORT_DEMO'; end if;
  ...
  select exists (select 1 from public.reports where reporter_id = v_reporter and target_type = p_target_type and target_id = p_target_id and status = 'open') into v_my_open;
  ...
  if (v_severity = 'severe' and v_credible and v_my_open) or (v_severity = 'mild' and v_distinct >= 3) then
...
-- 4) Grants: recompute stays server-only; submit_report stays member-callable.
