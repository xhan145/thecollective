...
The ranking logic remains in `lib/feedAlgorithm.ts` and keeps rewarding usefulness, actionability, proof strength, trust, recency, contribution fit, low friction, and mode.

The proof feed MVP uses `rankProofFeed` in `lib/proofData.ts`: recent proof submissions first, boosted when they have no feedback yet, and boosted by consistent practice. It does not rank by likes, followers, virality, or popularity.
...
- V9 separates photo proof and video proof into adjacent swipe lanes. Photo proof should feel quick and concrete. Video proof should feel richer, more deliberate, and more tightly bounded by the user's feedback request.
- V9 engagement controls should capture intent, not popularity. Reflecting, asking for context, trying the practice, saving a prompt, and giving focused feedback are useful signals. Generic likes are not.
