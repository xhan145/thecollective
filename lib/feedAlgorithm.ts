...
export function rankHomeFeed(items: FeedItem[], user: DemoUser): RankedFeedItem[] {
...
export function enforcePassiveToActiveFlow(items: RankedFeedItem[]) {
  ...
  const active = items.filter((i) => i.mode === "active");
  ...
  while (passive.length || bridge.length || active.length) {
    const lastTwoPassive = out.slice(-2).every((i) => i?.mode === "passive");
    const next = lastTwoPassive ? take(active) || take(bridge) || take(passive) : take(bridge) || take(active) || take(passive);
    if (!next) break;
    out.push(next);
  }
  return out;
...
export function feedOperatingPrinciples() {
  return [
    "The homepage may feel scrollable, but every card must point toward practice, proof, feedback, trust, or contribution.",
...
export function mediaProofScoringNotes() {
  return [
    "Video proof can carry high proof strength because it shows effort and context, but it also has higher friction and should not dominate the feed.",
