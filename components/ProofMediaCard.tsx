...
export function ProofTypeBadge({ proofType, mediaKind }: { proofType: ProofType; mediaKind?: MediaKind }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.07] px-2.5 py-1 text-[11px] font-black text-purple2">
      <Icon size={13} />
    ...
    <div className="rounded-[26px] border border-white/10 bg-white/[0.04] p-3">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-white/10 text-purple2">
            {proofType === "screenshot" ? <Monitor size={17} /> : <Icon size={17} />}
          </div>
          <div className="min-w-0">
            <p className="truncate text-sm font-black">{title}</p>
            <p className="text-xs text-[#8f887e]">{fileType}{activeMedia?.fileSize ? ` - ${formatFileSize(activeMedia.fileSize)}` : ""}</p>
          </div>
        </div>
      ...
      {mediaKind === "image" && activeMedia?.fileUrl && (
        <img src={activeMedia.fileUrl} alt="Proof preview" className="max-h-64 w-full rounded-[22px] object-cover" />
      )}
      {mediaKind === "video" && activeMedia?.fileUrl && (
        <video src={activeMedia.fileUrl} controls className="max-h-64 w-full rounded-[22px] bg-black" />
      )}
      {mediaKind === "audio" && activeMedia?.fileUrl && (
        <audio src={activeMedia.fileUrl} controls className="w-full" />
      )}
      {mediaKind === "document" && (
        <div className="flex items-center gap-3 rounded-[22px] bg-ink/70 p-3 text-sm text-[#c8c2b8]">
      ...
      {mediaKind === "link" && proof?.linkUrl && (
        <div className="rounded-[22px] bg-ink/70 p-3">
          <p className="text-xs font-bold text-[#8f887e]">Link proof</p>
          <p className="mt-1 break-all text-sm text-[#c8c2b8]">{proof.linkUrl}</p>
        </div>
      )}
      {mediaKind === "checklist" && proof?.checklistItems?.length ? (
        <div className="space-y-2">
          {proof.checklistItems.map((item) => (
            <p key={item} className="rounded-[22px] bg-ink/70 px-3 py-2 text-sm text-[#c8c2b8]">{item}</p>
          ))}
        </div>
      ) : null}
    </div>
